...
Deno.serve(async (req: Request) => {
  if (
    req.method !== "POST" ||
    req.headers.get("x-bookora-worker-secret") !== workerSecret
  ) {
  ...
  const { data: schedules, error } = await admin
    ...
    .select(
      "id,business_id,agent_id,prompt,interval_minutes,next_run_at",
    )
    .eq("enabled", true)
    ...
    const { data: claimed } = await admin
      ...
      .update({
        ...
        next_run_at: new Date(
          now.getTime() +
            Number(schedule.interval_minutes ?? 1440) * 60_000,
        ).toISOString(),
