      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-lg min-w-[180px] text-center">
            {view === 'month' ? format(currentDate, 'MMMM yyyy') : view === 'week' ? `${format(startOfWeek(currentDate), 'MMM d')} - ${format(endOfWeek(currentDate), 'MMM d, yyyy')}` : format(currentDate, 'EEEE, MMM d, yyyy')}
          </span>
            ...
            {bookingsForDay(currentDate).length === 0 ? (
              ...
              <div className="space-y-2">
                {bookingsForDay(currentDate).map((b) => (
                  <div key={b.id} className="flex items-center gap-3 p-3 rounded-lg border border-gray-200 dark:border-gray-700">
                    <div className="w-16 text-right">
                      <p className="text-sm font-medium">{formatTime(b.start_time)}</p>
                    </div>
                    <div className="flex-1">
                      <p className="text-sm font-medium">{b.customer?.name}</p>
                      <p className="text-xs text-gray-500">{b.service?.name} · {b.staff?.name}</p>
                    </div>
            ...
            <div className="grid grid-cols-7">
              {days.map((day) => {
                ...
                return (
                  ...
                  >
                    <div className={`text-xs mb-1 ${isToday(day) ? 'font-bold text-primary-600' : 'text-gray-500'}`}>
                      {format(day, 'd')}
                    </div>
                    <div className="space-y-1">
                      {dayBookings.slice(0, 3).map((b) => (
                        <div
                          ...
                          style={{ backgroundColor: (b.service?.color || '#3b82f6') + '20', color: b.service?.color || '#3b82f6' }}
                      ...
                      {dayBookings.length > 3 && (
                        <div className="text-xs text-gray-400 px-1.5">+{dayBookings.length - 3} more</div>
                      )}
                    </div>
                  </div>
                )
